...
function getFactorial(n) {
  ...
  if (n) return n * getFactorial(n - 1);
  return 1;
}
...
function getDigitalRoot(num) {
  ...
  const sum = arr.reduce((val1, val2) => val1 + val2, 0);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    for (let f = 0; f < lengthConfig; f += 1) {
      if (bracketsConfig[f].includes(str[i])) {
        if (str[i] === bracketsConfig[f][1]
         && brackets[brackets.length - 1] === bracketsConfig[f][0]) {
          brackets.pop();
        } else {
          brackets.push(str[i]);
        }
...
function getCommonDirectoryPath(pathes) {
  ...
  while (truth) {
    const path = arr[0].shift();
    for (let i = 1; i < arr.length; i += 1) {
      truth = truth ? path === arr[i].shift() : truth;
    }
    if (truth) result += `${path}/`;
  }
  return result;
}
...
function getMatrixProduct(m1, m2) {
  ...
  throw new Error('Such matrices cannot be multiplied, since the number of columns of matrix m1 is not equal to the number of rows of matrix m2.');
}
